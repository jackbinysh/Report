...
where $F$ is a phenomenological scaling factor. We now have three free parameters, $F, \mu, \Theta$, which we may estimate as before. Based on the results of earlier fitting, we set a new bounding box for these parameters (shown in table \ref{Simplified_model_initial_bounding_box}) and run the CMA-ES 400 times.  Results are shown in Fig. \ref{SimplifiedModelResults}, for the \textit{13\_9} dataset only.
...
\end{figure}
 ...
 In contrast to the results of Fig. \ref{InitialResults}, we see relatively tightly estimated parameters, with no estimated parameter values near the initial bounding box. Further, this simplified model achieves error values as low as the more complex model. In fact, Fig \ref{13_9_f_simplified} shows that the simplified algorithm is more consistently able to find the minimal function value identified in Fig. \ref{InitialResults_13_9_f}, possibly because the low dimension of the new model makes parameter space easier to search.
 
 We also note that \ref{13_9_scatterplot_simplified} shows two distinct clusters of parameter sets, and a strong positive correlation between between $F$ and $\Theta$. The latter suggests that both play similar roles in determining the fit. Fig. \ref{Sensitivity_scaled_simplified} shows the normalised sensitivity curves using this simplified model for the \textit{13\_9} dataset, and also suggests $F$ and $\Theta$ play similar, though not identical, roles.
  ...
  The clustering of values suggests that there may be several local minima, all with similar error values. We further investigate by fixing the value of $\Theta$, and plotting the error as a function of $\mu$ and $F$.
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Inter-parameter correlation values, and error function values, for the \textit{13\_9} and \textit{14\_7} datasets.}     
