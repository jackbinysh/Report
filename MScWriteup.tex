...
\begin{figure*}
    ...
    \centering
        \includegraphics[scale = 0.3, clip = true, trim = 80 0 74 0]{13_9_hist.eps}
    ...
    \end{subfigure}
    \caption{Parameter estimates, inter-parameter correlation values, and model predictions for the \textit{13\_9} and \textit{14\_7} datasets. Note that in the model predictions, aTc forcing is shown - IPTG concentration is constant at a level which saturates the cell's response. The forcing curve's height is schematic - aTc concentration is switched between off, and a level which saturates the cell's response \todo[inline]{colour forcings differntly, the 14 7 has the wrong forcing plotted, put a colorbar on the maps.}}
